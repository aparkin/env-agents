#!/usr/bin/env python3
"""
Test if reusing the same SRTM adapter across multiple clusters causes issues
"""
import sys
import sqlite3
import time
...
print(f"📂 Testing adapter reuse with {len(clusters)} clusters")

# Create SINGLE adapter instance (like production script caching)
EARTH_ENGINE = CANONICAL_SERVICES["EARTH_ENGINE"]
adapter = EARTH_ENGINE(asset_id="USGS/SRTMGL1_003")
print(f"✅ Created shared SRTM adapter")
...
for cluster_id, center_lat, center_lon in clusters:
    ...
    try:
        ...
        if result and len(result) > 0:
            ...
            print(f"      Variable: {result[0].get('variable')}, Value: {result[0].get('value')}")
...
# Summary
print(f"\n{'='*60}")
print(f"📊 ADAPTER REUSE TEST SUMMARY")
print(f"{'='*60}")
...
if no_data_count > 0 or error_count > 0:
    print(f"\n🔍 DIAGNOSIS: Adapter reuse is causing failures!")
    print(f"   Production script caches adapters - this may be the issue")
else:
    print(f"\n✅ Adapter reuse works fine - issue is elsewhere")